...
describe('save plugin', function () {
  it('should save scaffolds:', function (done) {
    ...
    app.resolve('doowb/scaffold-assemblefile', function (err, scaffolds) {
      ...
      app.save(scaffolds, function (err, scaffolds) {
        if (err) return done(err);
        assert(typeof app.get('doowb/scaffold-assemblefile') !== 'undefined');
        assert(typeof app.get('doowb/scaffold-assemblefile').options.repo !== 'undefined');
        assert.equal(app.get('doowb/scaffold-assemblefile').options.repo, 'doowb/scaffold-assemblefile');
        done();
      });
    });
  });

  it('should save scaffolds with name:', function (done) {
    var restore = capture(process.stdout);
    var app = new App();
    app.resolve('doowb/scaffold-assemblefile', function (err, scaffolds) {
      var output = restore(true);
      if (err) return done(err);
      delete scaffolds.options.repo;
      app.save(scaffolds, function (err, scaffolds) {
        if (err) return done(err);
        assert(typeof app.get('scaffold-assemblefile') !== 'undefined');
        assert(typeof app.get('scaffold-assemblefile').options.repo === 'undefined');
        done();
      });
    });
  });
